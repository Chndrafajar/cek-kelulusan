...
export default function EditSiswa() {
  ...
  const handleUpdateSiswa = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put(`https://cek-kelulusan-api.vercel.app/api/v1/siswa/update/${id}`, {
        nama,
        nis,
        status,
        tgllahir,
  ...
  const handleDeleteSiswa = async (e) => {
    try {
      const res = await axios.delete(`https://cek-kelulusan-api.vercel.app/api/v1/siswa/delete/${id}`);
      if (res && res.data.success) {
        navigate('/dashboard/data-siswa');
        swal('Good Job!', res.data.message, 'success');
      } else {
